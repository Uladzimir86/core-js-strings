...
function isString(value) {
  ...
  if (value && typeof value === 'object' && typeof value.valueOf() === 'string')
    return true;
  return false;
}
...
function removeFirstOccurrences(str, value) {
  ...
  return ind >= 0 ? str.slice(0, ind) + str.slice(ind + value.length) : str;
}
...
function orderAlphabetically(str) {
  let changedStr = str;
  let res = '';
  while (changedStr.length > 0) {
    let minInd = 0;
    let minEl = changedStr[0];
    for (let i = 1; i < changedStr.length; i += 1) {
      if (minEl > changedStr[i]) {
        minInd = i;
        minEl = changedStr[i];
      }
    }
    res += minEl;
    changedStr = changedStr.slice(0, minInd) + changedStr.slice(minInd + 1);
  }
  return res;
}
...
function countVowels(str) {
  const matchStr = str.match(/[aeyuio]/gi);
...
function isPalindrome(arg) {
  const str = arg.toLowerCase().match(/[\w]/g);
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strLength; i += 1) {
    if (str[i].match(/\w/)) {
      ...
      reverseStr += reverseString(word) + str[i];
      word = '';
    } else reverseStr += str[i];
  }
  if (word.length > 0) reverseStr += reverseString(word);
  return reverseStr;
}
...
function invertCase(str) {
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/[a-z]/)) res += str[i].toUpperCase();
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    let koef = 0;
    if (str[i].match(/[A-Ma-m]/)) koef = 13;
    else if (str[i].match(/[N-Zn-z]/)) koef = -13;
    res += String.fromCharCode(str.charCodeAt(i) + koef);
  }
  return res;
}
